...
void quit(GtkWidget *window, gpointer data);//시그널 종료
void product_register(GtkWidget *widget, gpointer data); //상품등록 대화상자 생성
void view_product_list(GtkWidget *widget, gpointer data);//상품목록 생성 
void supply_shipment(GtkWidget *widget, gpointer data);//상품조달 대화상자 생성
void view_delivery_list(GtkWidget *widget, gpointer data);//납품내역 생성
void *gtk(); // gtk메인
...
void send_msg(char * msg, int len) {   // send to all
	...
	for(i=0; i<clnt_cnt; i++)
		write(clnt_socks[i], msg, len); // 전체 클라이언트에 메세지뿌림
printf("아재요 출력됬어요:%d:%s",len,msg); // 서버에 메세지뿌리는데 글자수 제한있음
	pthread_mutex_unlock(&mutx);
}
...
//상품조달 대화상자 생성 함수------------------------------------------------------------------------
void supply_shipment(GtkWidget *widget, gpointer data){
    //위젯 선언----------------------------------------------------------------------
    GtkWidget *dialog;
    GtkWidget *market_label, *name_label, *number_label, *amount_label, *total_label;

    GtkWidget *market_entry, *name_entry,*number_entry, *amount_entry, *total_entry;
    GtkWidget *market_hbox, *name_hbox, *number_hbox, *amount_hbox, *total_hbox;
   
    gint result;  //대화상자 응답값 반환값을 저장하기위한 변수선언

    //대화상자 생성(제목텍스트, 부모위젯, 대화상자속성, 버튼이름, 응답코드, 버튼, 응답코드 ... 마지막에 NULL)
    //버튼이 포함된 대화상자 위젯 생성
    dialog = gtk_dialog_new_with_buttons("상품 조달", NULL, GTK_DIALOG_DESTROY_WITH_PARENT,
                        "납품", GTK_RESPONSE_YES, "취소", GTK_RESPONSE_NO, NULL);
    //라벨생성-------------------------------------------------------------------
    market_label = gtk_label_new("거래처 명  :");
    name_label = gtk_label_new("상품  명  :");
    number_label = gtk_label_new("상품번호 :");
    amount_label = gtk_label_new("수    량 :");
    total_label = gtk_label_new("총    액 :");
    
    //엔트리 위젯(한줄 입력 박스)생성-----------------------------------------------------------
    market_entry = gtk_entry_new();
    name_entry = gtk_entry_new();  //한줄의 텍스트를 입력할 수 있는 Widget
                                //GtkEntry를 생성하는 함수이다. 반환 타입은 GtkWidget의 포인터이다.
    number_entry = gtk_entry_new();
    amount_entry = gtk_entry_new();
    total_entry = gtk_entry_new();

    //박스생성----------------------------------------------------------
    market_hbox = gtk_hbox_new(TRUE, 5); 
    name_hbox = gtk_hbox_new(TRUE, 5);  //수평패킹상자 위젯 생성
    number_hbox = gtk_hbox_new(TRUE, 5);
    amount_hbox = gtk_hbox_new(TRUE, 5);
    total_hbox = gtk_hbox_new(TRUE, 5);

    //박스에 위젯 추가-----------------------------------------------------
    gtk_box_pack_start(GTK_BOX(market_hbox), market_label, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(market_hbox), market_entry, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(name_hbox), name_label, FALSE, FALSE, 5);  //H:왼쪽 V:위쪽 부터 위젯추가
                                                //(채울상자, 넣을위젯, T:다른위젯과사용가능한 공간 채움,
                                                //T:위젯들이 남은공간 채움, 위젯둘러싼 공간여백폭)
    gtk_box_pack_start(GTK_BOX(name_hbox), name_entry, FALSE, FALSE, 5);
    ...
    gtk_box_pack_start(GTK_BOX(amount_hbox), amount_label, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(amount_hbox), amount_entry, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(total_hbox), total_label, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(total_hbox), total_entry, FALSE, FALSE, 5);

    //다이얼로그에 박스 추가----------------------------------------------------
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->vbox), market_hbox, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->vbox), name_hbox, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->vbox), number_hbox, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->vbox), amount_hbox, FALSE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->vbox), total_hbox, FALSE, FALSE, 5);
...
void view_delivery_list(GtkWidget *widget, gpointer data){
    ...
    GtkListStore *store;
    GtkWidget *view;
    GtkTreeIter iter1, iter2, iter3, iter4, iter5, iter6, iter7, iter8, iter9, iter10, iter11, 
...
//상품조달 버튼-----------------------------------------------------------------------------
    GtkWidget *shipment_bt = gtk_button_new_with_label("상품 조달");  //상품등록 버튼 생성
    gtk_widget_set_size_request(shipment_bt, 200, 50);  //상품등록 버튼 크기 설정
    gtk_fixed_put(GTK_FIXED(frame), shipment_bt, 600, 500);  //상품등록 버튼을 Fixed에 부착
    gtk_widget_show (shipment_bt);  //상품등록 버튼 출력

    //상품조달버튼에 툴팁 부착-------------------------------------
    GtkTooltips *tooltips;  //툴팁선언
    tooltips = gtk_tooltips_new();  //툴팁생성 
    gtk_tooltips_set_tip(tooltips, shipment_bt, "거래처에 납품을 원할 시 버튼을 누르세요", NULL); //버튼에 툴팁 설정
                                                                                 //(툴팁명, 버튼명, 부착할 자료, 널값)

    //상품조달 버튼 클릭시 콜백함수 호출------------------------------------------------------------------
    g_signal_connect(GTK_OBJECT(shipment_bt), "clicked", GTK_SIGNAL_FUNC(supply_shipment), NULL);
...
//첫 구현화면 구성 위젯 생성-----------------------------------------------------------------
    ...
    */
    ...
    gtk_fixed_put(GTK_FIXED(frame), product_bt, 100, 50);
    gtk_widget_set_size_request(product_bt, 300, 40);

    delivery_bt = gtk_button_new_with_label("납품내역");  //매출전표 버튼생성
    gtk_fixed_put(GTK_FIXED(frame), delivery_bt, 500, 50);
    gtk_widget_set_size_request(delivery_bt, 300, 40);

//각 버튼클릭시 시그널처리 콜백함수 호출-----------------------------------------------------------------------
    //g_signal_connect(GTK_OBJECT(market_bt), "clicked", GTK_SIGNAL_FUNC(view_market_list), NULL);  
